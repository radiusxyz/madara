...
mod tests {
    ...
    fn should_cease_building_block_when_deadline_is_reached() {
        ...
        let mut proposer_factory = ProposerFactory::new(spawner.clone(), client.clone(), txpool.clone(), None, None);
    ...
    fn should_cease_building_block_when_block_limit_is_reached() {
        ...
        let mut proposer_factory =
            ProposerFactory::with_proof_recording(spawner.clone(), client.clone(), txpool.clone(), None, None);
        ...
        let block_limit = {
            let builder = client.new_block_at(genesis_header.hash(), Default::default(), true).unwrap();
    ...
    fn should_keep_adding_transactions_after_exhausts_resources_before_soft_deadline() {
        ...
        block_on(txpool.maintain(chain_event(client.expect_header(genesis_hash).expect("there should be header"))));
