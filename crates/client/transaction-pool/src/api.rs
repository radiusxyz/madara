...
impl<Client, Block> FullChainApi<Client, Block> {
    ...
    pub fn new(
        ...
        FullChainApi { client, validation_pool: Arc::new(Mutex::new(sender)), _marker: Default::default(), metrics }
    }
}
    ...
    Client::Api: TaggedTransactionQueue<Block>,
{
    ...
    fn block_id_to_number(&self, at: &BlockId<Self::Block>) -> error::Result<Option<graph::NumberFor<Self>>> {
        self.client.to_number(at).map_err(|e| Error::BlockIdConversion(e.to_string()))
    }

    fn block_id_to_hash(&self, at: &BlockId<Self::Block>) -> error::Result<Option<graph::BlockHash<Self>>> {
