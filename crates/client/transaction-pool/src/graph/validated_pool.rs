...
use sp_runtime::transaction_validity::{TransactionSource, TransactionTag as Tag, ValidTransaction};

use super::base_pool::{self as base, PruneStatus};
use super::listener::Listener;
use super::pool::{BlockHash, ChainApi, EventStream, ExtrinsicFor, ExtrinsicHash, Options, TransactionFor};
use super::rotator::PoolRotator;
...
pub type ValidatedTransactionFor<B> = ValidatedTransaction<ExtrinsicHash<B>, ExtrinsicFor<B>, <B as ChainApi>::Error>;
...
impl<B: ChainApi> ValidatedPool<B> {
    ...
    pub fn check_is_known(&self, tx_hash: &ExtrinsicHash<B>, ignore_banned: bool) -> Result<(), B::Error> {
    ...
    fn enforce_limits(&self) -> HashSet<ExtrinsicHash<B>> {
            ...
            || future_limit.is_exceeded(status.future, status.future_bytes)
        {
            ...
            let removed = {
                ...
                let removed =
                    pool.enforce_limits(ready_limit, future_limit).into_iter().map(|x| x.hash).collect::<HashSet<_>>();
    ...
    pub fn extrinsics_tags(&self, hashes: &[ExtrinsicHash<B>]) -> Vec<Option<Vec<Tag>>> {
        self.pool
            ...
            .by_hashes(hashes)
            .into_iter()
            .map(|existing_in_pool| existing_in_pool.map(|transaction| transaction.provides.to_vec()))
            .collect()
    }
    ...
    pub fn fire_pruned(
        ...
        let header_hash =
            self.api.block_id_to_hash(at)?.ok_or_else(|| error::Error::InvalidBlockId(format!("{:?}", at)))?;
        let mut listener = self.listener.write();
    ...
    pub async fn on_block_finalized(&self, block_hash: BlockHash<B>) -> Result<(), B::Error> {
        log::trace!(target: LOG_TARGET, "Attempting to notify watchers of finalization for {}", block_hash,);
