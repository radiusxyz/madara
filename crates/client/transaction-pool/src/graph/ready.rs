...
impl<Hash: hash::Hash + Member + Serialize, Ex> ReadyTransactions<Hash, Ex> {
    ...
    pub fn by_hashes(&self, hashes: &[Hash]) -> Vec<Option<Arc<Transaction<Hash, Ex>>>> {
        ...
        hashes.iter().map(|hash| ready.get(hash).map(|x| x.transaction.transaction.clone())).collect()
    }
    ...
    fn remove_subtree_with_tag_filter(
        ...
        while let Some(hash) = to_remove.pop() {
            if let Some(mut tx) = ready.remove(&hash) {
                let invalidated =
                    tx.transaction.transaction.provides.iter().filter(|tag| {
                        provides_tag_filter.as_ref().map(|filter| !filter.contains(&**tag)).unwrap_or(true)
                    });
    ...
    pub fn prune_tags(&mut self, tag: Tag) -> Vec<Arc<Transaction<Hash, Ex>>> {
        ...
        while let Some(tag) = to_remove.pop() {
            let res = self.provided_tags.remove(&tag).and_then(|hash| self.ready.write().remove(&hash));
            ...
            if let Some(tx) = res {
                ...
                // prune previous transactions as well
                {
                    ...
                    let mut find_previous = |tag| -> Option<Vec<Tag>> {
                        ...
                        if tx2.unlocks.is_empty() { Some(tx2.transaction.transaction.provides.clone()) } else { None }
                    };
    ...
    fn replace_previous(
        ...
        let (to_remove, unlocks) = {
            ...
            let replace_hashes =
                tx.provides.iter().filter_map(|tag| self.provided_tags.get(tag)).collect::<HashSet<_>>();
            ...
            let unlocks = {
                let ready = self.ready.read();
                replace_hashes.iter().filter_map(|hash| ready.get(hash)).fold(vec![], |mut list, tx| {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if self.invalid.contains(hash) {
                debug!(target: LOG_TARGET, "[{:?}] Skipping invalid child transaction while iterating.", hash,);
                continue;
            }
            ...
            for hash in &ready.unlocks {
                ...
                // then get from the pool
                    ...
                    self.all.get(hash).map(|next| (next.requires_offset + 1, next.transaction.clone()))
                };
