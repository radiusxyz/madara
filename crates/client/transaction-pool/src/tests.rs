...
impl ChainApi for TestApi {
    ...
    fn validate_transaction(
        ...
        let res = match uxt {
            Extrinsic { function: RuntimeCall::Balances(BalancesCall::transfer_allow_death { .. }), .. } => {
            ...
            Extrinsic { function: RuntimeCall::SubstrateTest(PalletCall::include_data { .. }), .. } => {
                Ok(ValidTransaction {
                    priority: 9001,
                    requires: vec![],
                    provides: vec![vec![42]],
            ...
            Extrinsic { function: RuntimeCall::SubstrateTest(PalletCall::indexed_call { .. }), .. } => {
                Ok(ValidTransaction {
                    priority: 9001,
                    requires: vec![],
                    provides: vec![vec![43]],
